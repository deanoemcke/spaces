...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        chrome.extension.getBackgroundPage().spaces.requestCurrentSpace(function (space) {
            ...
            globalTabId = chrome.extension.getBackgroundPage().utils.getHashVariable('tabId', window.location.href);
            var url = chrome.extension.getBackgroundPage().utils.getHashVariable('url', window.location.href);
            globalUrl = url !== '' ? decodeURIComponent(url) : false;
            var windowId = chrome.extension.getBackgroundPage().utils.getHashVariable('windowId', window.location.href);
            globalWindowId = windowId !== '' ? windowId : false;
            var sessionName = chrome.extension.getBackgroundPage().utils.getHashVariable('sessionName', window.location.href);
    ...
    function routeView() {
        ...
        var action = chrome.extension.getBackgroundPage().utils.getHashVariable('action', url);
        if (action === 'move') {
            renderMoveCard();
    ...
    function renderCommon() {
        document.getElementById('activeSpaceTitle').value = globalCurrentSpace.name ? globalCurrentSpace.name : UNSAVED_SESSION;
